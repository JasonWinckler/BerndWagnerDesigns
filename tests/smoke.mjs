...
assert(!landing.includes('welcome-popup'), 'landing page must open directly on the split-screen banner');
...
assert(landingStyles.includes('.choice-canvas{position:relative;width:100%;height:100%}'), 'banner canvas must fill the viewport');
assert(landingStyles.includes('object-fit:cover'), 'banner artwork must cover the full screen');
